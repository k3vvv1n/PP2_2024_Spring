...
def fill_surface(surf, current_color):
    ...
    surf.blit(text, (5, 5))
    pygame.draw.rect(
        surf, color["l-gray"], (0, 35, surf.get_width(), surf.get_height() - 35)
    )
    palette = get_palette()
    surf.blit(palette, (3, 38))
    button.draw_border(
        surf, 1, 1, surf.get_width() - 1, surf.get_height() - 1, color["black"]
    )
...
class Colorpick:
    def __init__(self, x, y, width, height, color):
        ...
        for i in range(self.pwidth):
            ...
            pygame.draw.rect(
                self.surface, color, (self.rad + i, 0, 1, int(height * 0.5))
            )
        self.p = 0
    ...
    def draw(self, surf):
        ...
        pygame.draw.rect(
            surf,
            (0, 0, 0),
            (center[0] - self.rad // 2 + 10, center[1] - 25, 5, self.rad + 5),
            1,
            border_radius=2,
        )
