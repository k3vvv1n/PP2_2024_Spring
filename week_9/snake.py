...
class Snake:
    def __init__(self):
        ...
        self.head = pygame.Rect(
            self.x, self.y, BLOCK_SIZE, BLOCK_SIZE
        )  # rectangle for the snake's head
        self.body = [
            pygame.Rect(self.x - BLOCK_SIZE, self.y, BLOCK_SIZE, BLOCK_SIZE)
        ]  # snake's body as a list of rectangles
    ...
    def update(self):
        ...
        for square in self.body:
            if (
                self.head.x == square.x and self.head.y == square.y
            ):  # checking for collision of the snake's head with its body
                self.dead = True
            if self.head.x not in range(0, SW) or self.head.y not in range(
                0, SH
            ):  # checking if the snake leaves the playing area
                self.dead = True
        ...
        if self.dead and self.restart:
            ...
            golden_apple = GoldenApple(
                self.body, (apple.x, apple.y), [barrier for barrier in wall.barriers]
            )  # creating a new golden apple
...
class GoldenApple:
    def __init__(self, snake_body, apple_pos, wall_barriers):
        ...
        if (
            random.random() <= 0.1
        ):  # call spawn_golden_apple if a random number is less than or equal to 0.1 (probability 10%)
    ...
    def spawn_golden_apple(self, snake_body, apple_pos, wall_barriers):
        while True:
            ...
            if (
                (self.x, self.y) not in apple_pos
                and (self.x, self.y)
                not in [(square.x, square.y) for square in snake_body]
                and (self.x, self.y)
                not in [(barrier.x, barrier.y) for barrier in wall_barriers]
            ):
                # creating a rectangle representing the golden apple
                self.golden_apple_rect = pygame.Rect(
                    self.x, self.y, BLOCK_SIZE, BLOCK_SIZE
                )
                break
    ...
    def update(self, snake_body, apple_pos, wall_barriers):
        ...
        if self.golden_apple_rect is not None:  # checking if a golden apple exists
            ...
            ):  # if it exists, checking if it's time to despawn it (3 seconds)
                self.golden_apple_rect = None
...
class Wall:
    ...
    def spawn_barrier(self, snake_body, apple_pos, snake_head_pos):
        while True:
            ...
            if (
                new_barrier.collidelist(snake_body) == -1
                and new_barrier.collidepoint(apple_pos) == False
            ):
                ...
                if (
                    abs(snake_head_pos[0] - new_barrier.x) > 3 * BLOCK_SIZE
                    or abs(snake_head_pos[1] - new_barrier.y) > 3 * BLOCK_SIZE
                ):
    ...
    def update(self, snake_body, apple_pos, snake_head_pos, eaten_fruits):
        for barrier in self.barriers:
            ...
            if barrier.colliderect(
                snake_head_pos
            ):  # checking for collision with added barriers
                snake.dead = True

        eaten_fruits = (
            eaten_fruits // 2
        )  # this means that a new barrier is added after every second fruit eaten.
...
wall = Wall()
golden_apple = GoldenApple(
    snake.body, (apple.x, apple.y), [barrier for barrier in wall.barriers]
)
...
while not done:
    ...
    pygame.draw.rect(
        screen, (42, 42, 42), [0, SH, WW, WH]
    )  # drawing the surface for showing some statistics
    ...
    if golden_apple.golden_apple_rect is not None and snake.head.colliderect(
        golden_apple.golden_apple_rect
    ):  # working like red apple, but score->3
    ...
    if snake.head.x == apple.x and snake.head.y == apple.y:
        ...
        if (
            len(snake.body) - 1
        ) % 5 == 0:  # this means that speed is increasing after every fifth fruit eaten.
            speed += 0.5

    # checking that a new apple will not be created on an existing barrier
    for barrier in wall.barriers:
        if (
            apple.x == barrier[0] and apple.y == barrier[1]
        ):  # else regenerate new position for apple
            apple.spawn_apple()
