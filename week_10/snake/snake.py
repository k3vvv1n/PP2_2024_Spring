...
sql = f"select * from snakedb where user_login =\'{login_name}\'"
cursor.execute(sql)
...
if len(info) > 0:
    ...
    sql_insert = f"INSERT INTO snakedb(user_login, last_score, last_level, last_FPS, snake_len, wall_len, record) VALUES( \'{login_name}\',\'{score_value}\',\'{level_value}\',\'{FPS}\', \'{snake_len}\', \'{wall_len}\',\'{record}\' )"
...
def GAME_OVER():
    ...
    sql_insert1 = f"UPDATE snakedb set  last_score = {score_value}, last_level ={level_value} , last_FPS = {FPS}, snake_len = {snake_len}, wall_len = {wall_len} where user_login = \'{login_name}\' "
    cursor.execute(sql_insert1)
    if score_value > record:
        sql_insert1 = f"UPDATE snakedb set record = \'{score_value}\'  where user_login = \'{login_name}\' "
        record = score_value
        cursor.execute(sql_insert1)
